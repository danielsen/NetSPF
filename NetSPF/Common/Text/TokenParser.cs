namespace NetSPF.Common.Text
{
    public abstract class TokenParser
    {
        ...
        protected delegate bool TryMakeDelegate<TOut1, TOut2, TOut3>(out TOut1 found1, out TOut2 found2,
            out TOut3 found3);
        ...
        /// <returns>true if the match could be made, false if not.</returns>
        protected bool TryMake<TOut1, TOut2>(TryMakeDelegate<TOut1, TOut2> @delegate, out TOut1 found1,
            out TOut2 found2)
        {
            var checkpoint = Enumerator.Checkpoint();
        ...
        protected bool TryMake<TOut1, TOut2, TOut3>(TryMakeDelegate<TOut1, TOut2, TOut3> @delegate, out TOut1 found1,
            out TOut2 found2, out TOut3 found3)
        {
            var checkpoint = Enumerator.Checkpoint();

            if (@delegate(out found1, out found2, out found3) == false)
            {
                checkpoint.Rollback();
                return false;
            }
