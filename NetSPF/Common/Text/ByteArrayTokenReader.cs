...
namespace NetSPF.Common.Text
{
    public sealed class ByteArrayTokenReader : TokenReader
    {
        ...
        private Token OtherToken(byte value)
        {
            ...
            {
                return Token.Create(TokenType.Space, value);
            }

            return Token.Create(TokenType.Other, value);
        }
        ...
        {
            return Token.Create(TokenType.Number, Consume(Token.IsNumber));
        }
        ...
        private Token NewLineToken()
        {
            ...
            return Token.Create(state == 2 ? TokenType.NewLine : TokenType.Space, text);
        }
        ...
        {
            return string.Concat(ConsumeIterator(predicate).Select(segment =>
        ...
        {
            if (_index >= _segments.Count || _position < _segments[_index].Count) return _index < _segments.Count;
            _index++;
            _position = 0;
