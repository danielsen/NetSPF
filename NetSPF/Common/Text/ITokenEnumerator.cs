...
namespace NetSPF.Common.Text
{
    ...
    public static class TokenEnumeratorExtensions
    {
        ...
        /// </summary>
        /// <param name="enumerator">The enumerator to perform the operation on.</param>
        /// <param name="type">The token kind to test against to determine whether the token should be skipped.</param>
        public static void Skip(this ITokenEnumerator enumerator, TokenType type)
        {
            if (enumerator == null)
            {
                throw new ArgumentNullException(nameof(enumerator));
